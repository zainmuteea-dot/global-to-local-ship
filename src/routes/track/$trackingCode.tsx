    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#fdfbf7]" dir="rtl">
        <div className="bg-white p-8 rounded-2xl border text-center">
          <h2 className="font-black text-xl">لم يتم العثور على الشحنة</h2>
          <p className="text-zinc-500 mt-2">{trackingCode}</p>
          <a href="/track" className="mt-4 inline-block bg-zinc-900 text-white px-6 py-2 rounded-full">جرب كود آخر</a>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#fdfbf7] p-6" dir="rtl">
      <div className="max-w-2xl mx-auto">
        <div className="bg-zinc-900 text-white p-6 rounded-2xl">
          ...
          <h1 className="text-3xl font-black mt-1 tracking-widest">{order?.tracking_code}</h1>
        ...
        <div className="bg-white mt-6 p-6 rounded-2xl border">
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="flex justify-between"><span className="text-zinc-500">التاريخ</span><span className="font-bold">{order?.created_at? new Date(order.created_at).toLocaleDateString('ar-EG') : "—"}</span></div>
          </div>
        </div>
      </div>
    </div>
